...
<aside id="mobile-sidebar" class="w-64 bg-brand-dark h-full flex flex-col border-r border-gray-800 shrink-0 fixed inset-y-0 left-0 z-[100] transform -translate-x-full transition-transform duration-300 md:relative md:translate-x-0 md:flex">
    <div class="p-6 relative">
        ...
        <div class="mb-4">
            ...
            <nav class="space-y-1">
                <a href="{{ route('anime-board.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg transition-colors {{ request()->routeIs('anime-board.index') ? 'bg-brand-card text-white hover:bg-brand-card/80' : 'text-gray-400 hover:text-white hover:bg-brand-card/50' }}">
                    <svg class="w-5 h-5 {{ request()->routeIs('anime-board.index') ? 'text-brand-accent' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path>
                    </svg>
                    <span class="text-sm font-medium">My List</span>
                </a>
                <a href="{{ route('anime-board.browse') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg transition-colors {{ request()->routeIs('anime-board.browse') ? 'bg-brand-card text-white hover:bg-brand-card/80' : 'text-gray-400 hover:text-white hover:bg-brand-card/50' }}">
                    <svg class="w-5 h-5 {{ request()->routeIs('anime-board.browse') ? 'text-brand-accent' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                    </svg>
                    <span class="text-sm font-medium">Browse Anime</span>
                </a>
                <a href="{{ route('anime-board.favourites') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg transition-colors {{ request()->routeIs('anime-board.favourites') ? 'bg-brand-card text-white hover:bg-brand-card/80' : 'text-gray-400 hover:text-white hover:bg-brand-card/50' }}">
                    <svg class="w-5 h-5 {{ request()->routeIs('anime-board.favourites') ? 'text-brand-accent' : '' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
