<x-anime-layout>
    ...
    <script>
        ...
        function toggleFav(id, btnElement) {
            fetch(`/anime/${id}/toggle-favourite`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
            ...
            .then(data => {
                if(data.success) {
                    ...
                    if(data.is_favourite) {
                        btnElement.classList.remove('bg-black/60', 'text-gray-400', 'hover:text-white', 'hover:bg-black/80');
                        btnElement.classList.add('bg-yellow-500/20', 'text-yellow-400');
        ...
        function toggleCollectionFav(groupName, btnElement) {
            fetch(`/anime/collection/toggle-favourite`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
            ...
            .catch(error => {
                console.error('Error toggling collection favourite:', error);
            });
        }
    </script>
</x-anime-layout>
